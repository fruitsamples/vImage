...
- (void)setKernelType: (BOOL) isFP type: (int)kernelType
{
    int set = filter >> 16;
	...
	if (isFP)
		type = k->list[kernelType].typeFP;
	else
		type = k->list[kernelType].typeInt;
}

- (void)setSigmaValue:(float)value
{
	sigma = value;
}
...
-(void)applyInitFunction
{
    ...
    switch( type )
    {
        case kSInt16KernelType:
			if ( NULL != data )
				*(float *)data = sigma;			// for Richardson-Lucy
        case kUInt8KernelType:
        case kSInt8KernelType:
        case kUInt16KernelType:
        case kUInt32KernelType:
