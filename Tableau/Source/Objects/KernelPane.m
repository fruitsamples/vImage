...
- (void)initObject
{
    ...
    for( set = 0; set < kListCount; set++ )
    {
        ...
        for( item = 0; item < filterLists[set].count; item++ )
        {
			...
			filterLists[set].list[item].lastIntKernel = NULL;
			filterLists[set].list[item].lastIntKernel = NULL;
			
			[ newKernel setSigmaValue: [ parameters params ]->sigmaValue ];
			[ newKernelFP setSigmaValue: [ parameters params ]->sigmaValue ];
...
{
    Kernel *currentKernel = [ kernelData dataSource ];
	int		filter = [controller filter];
    int	set = filter >> 16;
    int item = filter & 0xFFFF;
	FilterInfo *info = &filterLists[set].list[item];
    
    if( nil != currentKernel )
    {
        int function = [ kernelPrefabType indexOfSelectedItem ];
        
        [ kernelData deselectAll: nil ];
        [ currentKernel setInitFunction: function];
		[ currentKernel setKernelType: [self isShowingFP] type: function ];
		[ currentKernel setSigmaValue: [ parameters params ]->sigmaValue ];
		...
		if ([self isShowingFP])
			info->lastFPKernel = currentKernel;
		else
			info->lastIntKernel = currentKernel;
    }
}
...
{
	[intText setHidden: (dataType == planar_F || dataType == ARGB_FFFF)];
	[FPText setHidden: (dataType == planar_8 || dataType == ARGB_8888)];

	[self setIntOrFloat: self];
}
...
- (void)setFilter:(int)filter
{
	...
	info = &filterLists[set].list[item];
    if( [ self isShowingFP ] ) {
		if (info->lastFPKernel)
			currentKernel = info->lastFPKernel;
		else {
		...
		if (info->lastIntKernel)
			currentKernel = info->lastIntKernel;
		else {
